...
def run_review_gate(
...
) -> tuple[bool, list[tuple[str, SkillResult]], str | None]:
    """
    작성 → 리뷰 → 재작업 루프 실행.

    Returns:
        (passed, review_results, latest_artifact_path)
        latest_artifact_path: 재작업이 있었으면 최종 산출물 경로, 없었으면 None
    """
    review_results = []
    current_reviewer_skills = list(reviewer_skills)
    ...
    for attempt in range(MAX_RETRIES + 1):
        ...
        # 리뷰어 병렬 호출
        review_results = invoke_parallel_skills(
            current_reviewer_skills, state, phase
        )
        ...
        # 재작업 산출물로 리뷰 대상 파일 갱신
        if rework_result.output_file:
            latest_artifact = rework_result.output_file
            current_reviewer_skills = [
                (skill_name, rework_result.output_file)
                for skill_name, _ in reviewer_skills
            ]
