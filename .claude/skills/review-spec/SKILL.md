...
> 시나리오의 § 섹션 ↔ L2 (UI 영역) 매핑이 기획서 § 구조와 일치하는지 확인.
> 기획서에 있는 § 단위 기능이 시나리오/TC에 빠짐없이 반영되었는지 검증.
...
3. `category`가 `coverage`, `business_logic`, `spec_gap`인 규칙만 선택 (이 스킬의 관심 영역)
...
| 규칙 필드 | 리뷰 적용 |
|-----------|-----------|
| `pattern` | 체크 항목으로 추가 (예: "결제 취소 후 환불 정책 TC 누락 여부") |
...
- 저장 경로: `data/reviews/{PROJECT}_{version}_{feature}_review-spec.json`
      ...
      "findings": [
        {
          "severity": "높음",
          "type": "누락",
          "detail": "비밀번호 재설정 플로우 시나리오 누락",
          "spec_reference": "기획서 3.2절"
        }
      ]
    },
    {
