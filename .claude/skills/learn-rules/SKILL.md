...
> **본 스킬은 `.claude/rules/tc_writing_rule.md`(양식 표준)과 별개 레이어로 동작.**
> - **양식 규칙** (`.claude/rules/tc_writing_rule.md`): 구조·분류·ID·분해·번호 매김 등 — 양식 표준. learn-rules가 다루지 않음.
> - **학습 규칙** (`data/rules/rule_*.json`): 케이스별 검증 룰 (모달 ESC·뒤로가기 등) — learn-rules가 추출·관리.
> - **충돌 방지**: 학습 규칙 추출 시 양식 관련 패턴(예: "L2에 카테고리 쓰지 말기")은 추출 제외 — 이미 양식 규칙에 정의됨.
> - **양식 변경 시**: 양식 규칙 갱신은 `tc_writing_rule.md` 직접 수정. learn-rules는 양식 외 영역(엣지케이스·네거티브·UX 리스크 등)만 학습.
