...
3. 모든 카테고리의 규칙을 선택 (TC 내용 리뷰는 커버리지 + 엣지케이스 모두 해당)
...
| 규칙 필드 | 리뷰 적용 |
|-----------|-----------|
| `pattern` | TC 누락 여부 체크 항목으로 추가 |
| `recommendation` | 누락 발견 시 구체적 TC 추가 제안에 활용 |
...
| 3 | **성공/실패 케이스 모두 포함** | 모든 동작에 성공 케이스와 실패 케이스 양쪽 존재 | 높음 |
...
- **동일 Depth 값 중복 검출**: depth_1~3에서 같은 메뉴/화면/영역명이 2개 이상 존재하면 지적 (TC 수정 시 빈번 발생)
...
```json
{
  ...
  "missing_tc": [
    ...
    },
    {
      "id": "M-002",
      "type": "실패 케이스 누락",
      "checklist_item": "성공/실패 케이스 모두 포함",
      "location": "AI 가이드 등록",
...
3. 커버리지 체크리스트 검토 (섹션 2.1~2.4) ← 메인
   → 입력 필드, 버튼, 성공/실패 케이스 모두 포함, 에러 케이스 누락 확인
