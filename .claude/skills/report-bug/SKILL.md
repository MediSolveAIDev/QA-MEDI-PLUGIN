...
| 영향 버전 (versions) | 구글 시트 M컬럼 / 폴백 | **시트 자동 → 폴백 수동** | 아래 버전 결정 규칙 참조 |
| 우선순위 | FAIL 패턴 분석 | **AI 추천** + 팀장 컨펌 | 재현 빈도, 영향 범위 기반 |
| 담당자 (assignee) | 팀장 지정 | **건별 수동** | 팀장이 이슈별로 직접 지정 (예: "이거 홍길동") |

> **영향 버전 결정 규칙** (우선순위 순):
> 1. 시트 FAIL 검출 시 → M컬럼(테스트 버전) 값 사용 (자동)
> 2. M컬럼 비어있거나 수동 버그 등록 시 → `config/project.json`의 `version` 참조하여 제안
> 3. 위 모두 불가 시 → 팀장에게 직접 질문
>
> JIRA API 필드: `"versions": [{"name": "v1.4.0"}]`
...
┌──────────────────────────────────────────────┐
│  {PROJECT} 버그 현황 리포트                    │
│  {컴포넌트} {version} (rc.N~rc.N)             │
│  생성일: {날짜}                                │
├──────────────────────────────────────────────┤
│  [ROW 1] 생성 대비 해결 추이 (full-width)      │
│  ┌────────────────────────────────────────┐  │
│  │ 라인 차트: 생성(누적) vs 해결(누적)      │  │
│  └────────────────────────────────────────┘  │
├──────────────────────────────────────────────┤
│  [ROW 2] 2-column                            │
│  ┌──────────┐ ┌──────────┐                   │
...
**삭제된 항목** (이전 규격 대비):
- 요약 카드 4개 (총 버그/미해결/해결률/고질적 이슈) — 삭제
- QA 분석 섹션 (릴리즈 리스크/취약 영역/고질적 이슈/장기 체류/미배정) — 삭제
- 컴포넌트별 바 차트 — 삭제

**추가된 항목:**
- 배포버전(rc) x 컴포넌트 히트맵 (heat-0~heat-4 색상 단계)
- 레이블별 현황 테이블 (디자인QA / automation / 환경이슈 / Later / 없음)
- 테이블에 rc 버전 컬럼 (영향 버전 = affectedVersion에서 rc 번호 추출)
- "진행 중" 상태 구분 (미해결/진행 중/해결됨/종료 4단계)

**히트맵 규칙:**
- 행: rc 버전 (rc.1, rc.2, ..., rc.N)
- 열: 컴포넌트 (Admin / 에이전트PC 등)
- 색상 단계: heat-0(0건, 회색) → heat-1(1건, 연노랑) → heat-2(2~3건, 주황) → heat-3(4~6건, 연빨강) → heat-4(7건+, 빨강)
- 마지막 행: 합계 (진한 배경)

**HTML 생성 규칙:**
- Chart.js는 CDN URL을 `<script>` 태그로 인라인 포함
- 도넛 차트 중앙에 전체 건수 표시 (centerText 플러그인)
- 우선순위 바 차트는 가로(horizontal) 방향
- 테이블은 컬럼 헤더 클릭으로 정렬 가능 (JS 인라인)
- 티켓 키는 JIRA URL 링크 (`config/common.json`의 `jira.base_url` + `/browse/{key}`)
- 미해결/진행 중 행은 노란색 배경 하이라이트 (row-open 클래스)
- 색상 코딩: Highest=빨강, High=주황, Medium=노랑, Low=회색
- 상태 색상: 미해결=빨강, 진행 중=주황, 해결됨=파랑, 종료=녹색
- 태그 뱃지: ENV(환경이슈), Later(보류), Design(디자인QA), DUP(중복), Migration, BE Fix 등
